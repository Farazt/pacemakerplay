...
public class PacemakerAPITest extends WithApplication{
    ...
    public void POSTOnApiUsersRouteCreatesUser() {
      ...
      //ensure the database has one+4 user after exercising tests
	...
	@Test
	public void POSTOnApiActivityLocationRouteCreateLocation(){
		  if(addActivityToUser().status()== OK){
			  System.out.println("Added Activity: "+ Activity.findAll());
			  assertThat(addLocationToActivity().status(), equalTo(OK));
		  }
	}
    ...
    private Result addLocationToActivity(){
    	 RequestBuilder activityRequest = new RequestBuilder()
                 .method(POST)
                 .uri("/api/activities/1/locations")
                 .bodyJson(Json.parse(Fixtures.activityJson));
         return route(activityRequest);
    }
}
